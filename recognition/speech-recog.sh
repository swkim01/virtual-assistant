...
arecord -D $hardware -t wav -d $duration -r 16000 | flac - -f --best --sample-rate 16000 -o /dev/shm/out.flac 1>/dev/shm/voice.log 2>/dev/shm/voice.log; echo "{\"config\": {\"encoding\":\"FLAC\",\"sampleRateHertz\":16000,\"languageCode\":\"ko-KR\"},\"audio\": {\"content\":\"" > /dev/shm/out.json; base64 /dev/shm/out.flac >> /dev/shm/out.json; echo "\"}}" >> /dev/shm/out.json; curl -X POST --data @/dev/shm/out.json --user-agent 'Mozilla/5.0' --header 'Content-Type: text/json' "https://speech.googleapis.com/v1/speech:recognize?key=<KEY>" | sed -e 's/[{}]/''/g' | awk -F":" '{print $2}' | awk -F"[" '{print $1}' | tr -d '\n'
